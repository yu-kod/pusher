...
/** 宣言の拍で選べる2つ（docs/spec.md §3 ①）。どちらも「宣言した」として扱う */
export type DeclarationView =
  /** 投入する — レーン1つと手札1枚 */
  | { kind: "insert"; laneIndex: number; handIndexes: number[] }
  /** 降りる — 未確定得点を確定して、そのラウンドから抜ける */
  | { kind: "withdraw" };

/** ティックの中でのプレイヤー1人。`GameView.players` と同じ席順に並ぶ */
...
/** 押し出しの判定（docs/spec.md §3 手順3） */
export type RollOutcome = "success" | "failure" | "sideHole";

/**
 * 解決したレーン1本ぶん。
 *
 * **何が落ちたかは届かない。** 落ちたカードが分かると、滞留とレーンの中身を
 * 推測する材料になる（docs/spec.md §8）。枚数と点数だけを受け取る。
 */
...
export type TickView = {
  ...
  index: number;
  phase: TickPhase;
  /** いまの拍が終わる時刻（サーバーの epoch ミリ秒）。権威はここで、端末の時計は表示にしか使わない */
  deadlineAt: number;
  /** 解決の再生を始める時刻。再生の開始点をここに揃える（§8-5）。宣言の拍では null */
  resolvedAt: number | null;
  /** 先行権順に並んだ解決のステップ列。解決前は空 */
  steps: ResolutionStepView[];
  /**
   * 解決する順に並んだ席の添字（先行権・docs/spec.md §3）。
   *
   * 卓上ではプレイヤーカードの列がこれにあたる公開情報。順番を決めるのは
   * サーバー（`backend/src/game/priority.ts`）で、画面は並べるだけ。
   */
  order: number[];
  /** 席順。`GameView.players` と同じ並び */
...
export type RoomView = {
  ...
  players: RoomPlayer[];
  game: GameView | null;
  /** 3拍の進行（docs/realtime.md §8）。ロビーのあいだは null */
  tick: TickView | null;
};
