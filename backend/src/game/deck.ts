...
/**
 * デッキ構成の既定値（docs/spec.md v0.3 §1、採用案 A''）。
 *
 * 総枚数 104 枚。
 * ポイント制により落下カードが山札へ戻るため、場に出ている枚数
 * （レーン 3×12 + 滞留 3×9 + 手札 4×5 = 83枚）を上回っていれば足りる。
 *
 * - イベント 16 枚（全体の約 15%）。4種に 5/5/3/3
 * - コイン 88 枚（コイン札の中で 45.5 / 39.8 / 14.8）
 *
 * v0.2 から14枚増えているのは、**レーンを深くしたぶん**（奥5枚 → 12枚）。
 * ボール札が1ゲームに35回落ちて「大物」にならなかったのを、深さで落ち着かせた
 * （`docs/turn-structure.md` §9）。カードを増やさない案（A'）を採るなら
 * `V02_DECK_CONFIG` のまま初期滞留を 9 → 5 に下げて同じ深さにできる。
 *
 * 3コイン札の比率は #53 のシミュレーションで 25% → 約15% に下げた。
 ...
 * v0.2 のデッキ構成（90枚）。
 *
 * 比較のためだけに残してある（プリセット `v02`）。レーンが奥5枚だった頃の枚数で、
 * いまの深さ（12枚）には足りない。
 */
export const V02_DECK_CONFIG: DeckConfig = {
